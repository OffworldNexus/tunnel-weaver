...
async fn test_async_helpers() {
    let temp = TempDir::new().expect("tempdir");
    let db_path = temp.path().join("async.db");
    let backup_path = temp.path().join("async_backup.db");

    let store = Store::open(&db_path).expect("open failed");

    let config = Config {
        root_domain: "async.example.com".into(),
        admin_email: "async@example.com".into(),
        acme_provider: "letsencrypt".into(),
        listen_http: "127.0.0.1:80".parse().unwrap(),
        listen_https: "127.0.0.1:443".parse().unwrap(),
        control_socket: PathBuf::from("/run/weaver/async.sock"),
        acme_directory: None,
        acme_eab_kid: None,
        acme_eab_hmac: None,
        acme_root_ca_pem: None,
    ...
    store
        .save_config_async(config.clone())
        .await
        .expect("async save failed");

    let loaded = store.load_config_async().await.expect("async load failed");
    assert_eq!(loaded, config);

    // Read async
    let row_count: i64 = store
        .read_async(|conn| {
            let count = conn.query_row("SELECT count(*) FROM config", [], |r| r.get(0))?;
            Ok(count)
        })
        .await
    ...
    store
        .write_async(|conn| {
            conn.execute(
                "INSERT INTO cert_events (id, name, at, kind, detail) VALUES (1, 'cert1', 100, 'issued', 'ok')",
                [],
            )?;
            Ok(())
        })
    ...
    // Backup async
    store
        .backup_async(backup_path.clone())
        .await
        .expect("async backup failed");
